...
function update(): void {
    ...
    for (let i = coins.length - 1; i >= 0; i--) {
        if (player.collidesWith(coins[i])) {
            score++;
            createParticles(coins[i].x + coins[i].w / 2, coins[i].y + coins[i].h / 2, CONFIG.coin.color, CONFIG.particle.defaultCount);
            coins.splice(i, 1);
        }
    }
    ...
    for (let i = powerups.length - 1; i >= 0; i--) {
        if (player.collidesWith(powerups[i])) {
            ...
            createParticles(powerups[i].x + powerups[i].w / 2, powerups[i].y + powerups[i].h / 2, CONFIG.powerup.color, CONFIG.particle.defaultCount + 5);
            powerups.splice(i, 1);
        }
    }
}
